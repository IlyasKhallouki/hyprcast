...
int main(void){
  ...
  VASurfaceID nv12; CK(vaCreateSurfaces(va,VA_RT_FORMAT_YUV420,1280,720,&nv12,1,NULL,0),"alloc NV12");
  ...
  VAContextID ctx; CK(vaCreateContext(va,cfg,1280,720,VA_PROGRESSIVE,&nv12,1,&ctx),"vpp ctx");
  ...
  for(int i=0;i<N;i++){
    ...
    VARectangle r={0,0,1920,1080}; VARectangle ro={0,0,1280,720};
    p.surface=rgb; p.surface_region=&r; p.output_region=&ro;
